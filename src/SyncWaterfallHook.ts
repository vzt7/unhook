...
export function waterfall<T>(items: ((arg: T) => T)[]) {
  const next = (prevResult: T, index = 0): T => {
    ...
    const result = fn(prevResult);
    return next(result, index + 1);
  };
  return (arg: T) => next(arg);
}

export class SyncWaterfallHook<Fn extends (...args: any[]) => any> extends Hook<Fn> {
  dispatch(arg0: Parameters<Fn>[0]) {
